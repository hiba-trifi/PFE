...
if (isset($_POST["login"])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM members WHERE mb_email = ?");
        $stmt->execute([$email]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$row || !password_verify($password, $row["mb_pswrd"])) {
            $errors["password_inc"] = "Mot de passe saisi est incorrect";
        } else {
        ...
        <div class="container">
            ...
            <?php endif; ?>
                ...
                <div class="d-flex flex-column justify-content-center align-items-center mt-5">
                    ...
                    <div class="d-flex">
                        <p class="form_info">Vous n'avez pas de compte ?</p>
                        <a class="form_toogle mx-2" href="./Signup.php">Inscrivez-vous ici</a>
                    </div>
                </div>
            </form>
